...
class RGBVideoTransform:
    ...
    def __call__(self, video_tchw: torch.Tensor) -> torch.Tensor:
        transformed = self.transform(tv_tensors.Video(video_tchw))
        return torch.as_tensor(transformed).contiguous()
...
class RGBClipDataset(Dataset):
        ...
        if verify_path:
            ...
            if missing:
                raise FileNotFoundError(
                    f"Missing camera {self.camera_id} RGB tensor for examples: {missing}"
                )
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        if not torch.is_tensor(video) or video.ndim != 4 or video.shape[1] != 3:
            raise ValueError(
                f"Invalid RGB tensor for {row['sample_name']}: "
                f"{type(video)} {getattr(video, 'shape', None)}"
            )
...
def load_feature_cache(path: str | Path) -> dict[str, Any]:
    ...
    if not torch.is_tensor(cache["features"]) or cache["features"].ndim != 2:
        raise ValueError("Feature cache 'features' must be a two-dimensional tensor")
...
class FeatureHistoryDataset(Dataset):
    def __init__(
        ...
        for rows in grouped.values():
            ...
            for current_position, current_row in enumerate(rows):
                history_rows = list(rows[:current_position])
                if history_order == "graph_valid":
                    assert self.graph is not None
                    history_rows = randomized_graph_valid_history(
                        history_rows,
                        graph=self.graph,
                        seed=stable_sample_seed(
                            self.shuffle_seed,
                            str(current_row["sample_name"]),
                        ),
                    )
                ...
                self.examples.append(
                    HistoryExample(
                        ...
                        history_cache_indices=tuple(
                            cache_lookup[str(row["sample_name"])] for row in history_rows
                        ),
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        history_node_classes = torch.tensor(
            [int(row["node_idx"]) - 1 for row in example.history_rows],
            dtype=torch.long,
        )
        ...
        return {
            ...
            "history_sample_names": [
                str(history_row["sample_name"])
                for history_row in example.history_rows
            ],
        }
...
def collate_history_batch(batch: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    history_features = torch.zeros(
        (batch_size, max_history, feature_dim),
        dtype=torch.float32,
    )
