...
def confusion_martix(y_true: np.ndarray, y_pred: np.ndarray, num_classes: int) -> np.ndarray:
...
def classification_metrics(y_true: list[int], y_pred: list[int], num_classes: int) -> dict[str, Any]:
    ...
    return metrics_from_confusion(confusion_martix(true_array, pred_array, num_classes))


def aggregate_node_probabilities(
        node_probabilities: torch.Tensor, # [B, N]
        node_to_tier3: torch.Tensor, # [N]
        num_tier3: int,
) -> torch.Tensor:
    output_shape = (*node_probabilities.shape[:-1], num_tier3) # [B, M]
    result = torch.zeros(output_shape, device=node_probabilities.device, dtype=node_probabilities.dtype) # [B, M]
    index = node_to_tier3.view(*([1] * (node_probabilities.ndim - 1)), -1).expand_as(node_probabilities) # [1, N] -> [B, N]
