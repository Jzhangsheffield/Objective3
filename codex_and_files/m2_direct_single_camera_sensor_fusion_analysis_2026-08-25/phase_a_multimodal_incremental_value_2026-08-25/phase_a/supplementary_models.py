...
from .signal_backbones import build_signal_backbone


class SignalDirectClassifier(nn.Module):
    """One signal encoder and one independently trained Node or Tier3 head."""

    def __init__(self, config: dict, encoder: str, in_channels: int, num_classes: int) -> None:
        super().__init__()
        self.backbone = build_signal_backbone(config, encoder, in_channels)
        self.head = nn.Linear(int(self.backbone.feature_dim), num_classes)
    ...
    def forward_features(self, signal: torch.Tensor) -> torch.Tensor:
        return self.backbone.forward_features(signal)

    def forward(self, signal: torch.Tensor) -> torch.Tensor:
        return self.head(self.forward_features(signal))


class SensorM2Direct(nn.Module):
    """M2-Direct trained from scratch over frozen Tier3 signal feature caches."""
