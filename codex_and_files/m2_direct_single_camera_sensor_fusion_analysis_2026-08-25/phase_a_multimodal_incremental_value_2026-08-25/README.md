# Phase A：M2-Direct 多视角与 EMG/IMU 增量价值实验包
...
- `config/bilateral_supplementary_experiments.json`：双手 S1–S12、校准 run 与两种测试协议配置。
...
- `outputs/supplementary_bilateral/`：双手 S1–S12 的独立结果，不覆盖右手实验。
- `outputs/bilateral_signal_cache/`：双手 participant-specific train cache 和两种 test normalization cache。

## 双手 S1–S12 快速入口

统一脚本为 `scripts/run_all_bilateral_s1_s12.ps1`，其自动续跑、日志、失败控制、压力、延迟、bootstrap 与汇总功能和原 `run_all_phase_a.ps1` 一致。建议先使用 `-PlanOnly` 检查任务矩阵：

```powershell
& .\scripts\run_all_bilateral_s1_s12.ps1 `
  -Python 'C:\path\to\env\python.exe' `
  -Participants A -Seeds 1 -PlanOnly
```

默认完整运行 A/D/J/M × seed 1/2/42 × S1–S12。小范围运行时 bootstrap 会自动跳过，也可以显式使用 `-SkipBootstrap`。
