...
class CausalBoundaryStateMachine:
    ...
    def __init__(
        ...
        start_debounce_steps: int = 2,
        end_debounce_steps: int = 2,
    ...
    def reset(self) -> None:
        ...
        self.pending_segment: DetectedSegment | None = None
    ...
    def _emit_pending(self, index: int) -> DetectedSegment:
        if self.pending_segment is None:
            raise RuntimeError("No pending segment is available to emit")
        segment = self.pending_segment
        segment.emitted_at_index = max(segment.emitted_at_index, index)
            ...
            if start_evidence:
                ...
                if self.start_count >= self.start_debounce:
                    ...
                    if self.pending_segment is not None:
                        gap = self.current_start - self.pending_segment.end_index - 1
                        if gap <= self.merge_gap_steps:
                            self.current_start = self.pending_segment.start_index
                            self.current_start_score = max(
                                self.current_start_score, self.pending_segment.start_score,
                            )
                            self.pending_segment = None
                        else:
                            emitted.append(self._emit_pending(index))
            else:
                self.start_count = 0
                self.start_candidate = -1

            if self.mode == "background" and self.pending_segment is not None:
                latest_merge_start = self.pending_segment.end_index + self.merge_gap_steps + 1
                candidate_can_still_merge = (
                    self.start_count > 0 and self.start_candidate <= latest_merge_start
                )
                if index >= latest_merge_start and not candidate_can_still_merge:
                    emitted.append(self._emit_pending(index))
            return emitted
    ...
    def flush(self, final_index: int) -> list[DetectedSegment]:
        emitted: list[DetectedSegment] = []
        if self.pending_segment is not None:
            emitted.append(self._emit_pending(final_index))
        if self.mode == "action" and self.current_start >= 0:
            emitted.append(DetectedSegment(
                start_index=self.current_start,
