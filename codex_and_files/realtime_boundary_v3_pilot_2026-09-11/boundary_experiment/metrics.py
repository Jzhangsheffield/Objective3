...
def evaluate_run(
    ...
    if target_segment_ids is not None:
        gt_segments = []
        for sid in dict.fromkeys(np.asarray(target_segment_ids).tolist()):
            indices = np.flatnonzero((target_segment_ids == sid) & (target_state == 1))
            if len(indices):
                gt_segments.append((int(indices[0]), int(indices[-1])))
