...
class BoundaryChunkDataset(Dataset):
    def __init__(self, cache_root: str | Path, run_names: list[str], chunk_length: int, chunk_overlap: int):
        ...
        for name in run_names:
            ...
            for start in range(0, length, step):
                ...
                self.index.append((name, start, end, context_steps))
                if end == length:
                    break
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        name, start, end, context_steps = self.index[index]
...
def collate_chunks(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for i, row in enumerate(rows):
        ...
        context_steps = int(row.get("context_steps", 0))
        if not 0 <= context_steps <= length:
            raise ValueError(f"Invalid context_steps={context_steps} for chunk length={length}")
        features[i, :length] = row["features"]
        state[i, :length] = row["state"]
        start[i, :length] = row["start"]
        end[i, :length] = row["end"]
        mask[i, context_steps:length] = True
    return {
        "features": features,
        "state": state,
        "start": start,
        ...
        "context_steps": [int(row.get("context_steps", 0)) for row in rows],
    }
