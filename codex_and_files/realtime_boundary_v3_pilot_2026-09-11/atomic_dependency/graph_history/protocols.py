...
def _summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    node_support = {
        str(node_idx): sum(int(row["node_idx"]) == node_idx for row in rows)
        for node_idx in range(1, 36)
    }
    tier3_support = {
        str(tier3_id): sum(int(row["tier3_id"]) == tier3_id for row in rows)
        for tier3_id in range(31)
    }
    return {
        "samples": len(rows),
        "runs": len(run_keys),
        "participants": sorted({str(row["participant"]) for row in rows}),
        "node_idx_support": node_support,
        "tier3_id_support": tier3_support,
        "present_node_count": sum(value > 0 for value in node_support.values()),
        "present_tier3_count": sum(value > 0 for value in tier3_support.values()),
        "missing_node_idx": [int(key) for key, value in node_support.items() if value == 0],
        "missing_tier3_id": [int(key) for key, value in tier3_support.items() if value == 0],
    }
