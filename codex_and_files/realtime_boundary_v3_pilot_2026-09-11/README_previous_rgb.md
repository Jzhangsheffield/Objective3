...
本目录是一个与现有 M0–M6、Direct、Dynamic 和 Atomic-tail 输出完全隔离的新实验包。默认使用 `action_recognition_timestamps_canonical_v2` 标注：动作起止按源标注时间戳映射到排除无效占位帧后的参考相机序列，索引统一为 1-based、首尾帧均包含；动作间的短 background 保持显式存在。
...
- 新边界标注：`annotations/action_recognition_timestamps_canonical_v2`，包含每个 run 的 segmentation CSV 和逐帧 CSV。
