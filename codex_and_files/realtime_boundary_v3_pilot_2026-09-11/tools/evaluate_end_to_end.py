...
def _ground_truth_segments(table: dict, node_rows: list[dict], lineage: dict) -> list[dict]:
    result = []
    action_segment_ids = []
    ...
    if lineage["v2_action_count"] != len(node_rows):
...
def main() -> None:
    ...
    for split in args.splits:
        ...
        for run_position, run in enumerate(runs, 1):
            ...
            gt = _ground_truth_segments(table, node_rows[(info.participant, info.source_run)], lineage[run])
