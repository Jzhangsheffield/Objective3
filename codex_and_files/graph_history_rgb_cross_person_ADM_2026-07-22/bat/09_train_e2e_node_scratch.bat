...
if exist "%E2E_NODE_SCRATCH_OUTPUT%\completed.json" (
  echo Existing completed result found; skip without overwriting: %E2E_NODE_SCRATCH_OUTPUT%
  exit /b 0
)
"%PYTHON_BIN%" tools\train_e2e_node.py ^
  ...
  --output-dir "%E2E_NODE_SCRATCH_OUTPUT%" ^
  --init scratch ^
