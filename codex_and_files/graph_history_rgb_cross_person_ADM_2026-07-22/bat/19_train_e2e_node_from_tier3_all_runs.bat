...
if exist "%ALLRUN_E2E_NODE_TRANSFER_OUTPUT%\completed.json" (
  echo Completed all-runs E2E Node From Tier3 exists; skip without overwrite.
  exit /b 0
)
...
"%PYTHON_BIN%" tools\train_e2e_node.py ^
  ...
  --output-dir "%ALLRUN_E2E_NODE_TRANSFER_OUTPUT%" ^
  --init tier3 ^
  --init-checkpoint "%ALLRUN_BACKBONE_CKPT%" ^
