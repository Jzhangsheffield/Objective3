...
def classification_metrics(truth: list[int], prediction: list[int], classes: int) -> dict[str, Any]:
    ...
    for label in range(classes):
        ...
        per_class.append(
            {"class_id": label, "support": support, "precision": precision, "recall": recall, "f1": f1}
        )
...
def derive_node_to_tier3(rows: list[dict], nodes: int = 35) -> list[int]:
    ...
    if missing:
        raise ValueError(f"Missing node-to-Tier3 entries: {sorted(value + 1 for value in missing)}")
