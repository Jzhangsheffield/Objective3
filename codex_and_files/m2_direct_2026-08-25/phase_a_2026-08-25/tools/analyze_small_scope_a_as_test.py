...
def generate_node_impact_svg(rows: list[dict], path: Path) -> None:
    # Keep every modality on the same semantic x-axis: Node 1 -> Node 35.
    ...
    svg = [
        ...
        '<desc id="desc">上图是每个模型的绝对 Recall，下图是各候选相对 A0 的 F1 变化。类别按 Node 编号从 1 到 35 排列。</desc>',
        ...
        '<text class="note" x="40" y="79">类别按 Node 编号 N1→N35 排列；粗框表示 Recall &lt; 80%；单元格为百分比/百分点。</text>',
...
def generate_node_impact_png(rows: list[dict], path: Path) -> None:
    ...
    draw.text((40, 73), "类别按 Node 编号 N1→N35 排列；粗框表示 Recall < 80%；单元格为百分比/百分点。", fill="#333333", font=note_font)
...
def generate_sensor_node_heatmap(rows: list[dict], path: Path) -> None:
    """Absolute Node recall and F1 deltas for S1-S8, ordered by Node ID."""
    ordered = sorted(rows, key=lambda row: int(row["class_id"]))
    methods = ["A0"] + SENSOR_NODE_CONDITIONS
    candidates = SENSOR_NODE_CONDITIONS
    cell_width, cell_height = 112, 54
    # Match the A0-A6 Node figure exactly so N1-N35 occupy identical x positions
    # when the two images are rendered at the same width in the report.
    left, right, top = 500, 60, 145
    first_height = len(methods) * cell_height
    ...
    draw.text((40, 70), "按 Node 编号 N1→N35 排列并与 A0–A6 图逐列对齐；S1–S4 为 Tier3 encoder→冻结特征→M2 Node，S5–S8 为独立 Direct Node。", fill="#333333", font=note_font)
...
def generate_sensor_tier3_heatmap(rows: list[dict], path: Path) -> None:
    """Absolute Tier3 recall and F1 deltas for all S1-S12, ordered by Tier3 ID."""
    ordered = sorted(rows, key=lambda row: int(row["class_id"]))
    ...
    draw.text((40, 70), "类别按 Tier3 编号 T0→T30 排列；S1–S8 同时产生 Node，S9–S12 为独立 Direct Tier3。", fill="#333333", font=note_font)
    draw.text((40, top - 32), "绝对 Recall（%）", fill="#111111", font=subtitle_font)
    for row_index, method in enumerate(methods):
        y = top + row_index * cell_height
        draw.text((left - 15, y + cell_height / 2), f"{method}  {CONDITION_NAMES[method]}", fill="#111111", font=row_font, anchor="rm")
