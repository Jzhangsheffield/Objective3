...
class ResNet(nn.Module):
    ...
    def forward_features(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        if x.ndim != 5:
            raise ValueError(f"ResNet3D expects input [B,C,T,H,W], got shape={tuple(x.shape)}")
    ...
    def forward_head(self, feat: torch.Tensor) -> torch.Tensor:
        ...
        这样可以削弱 feature norm 对 logits 的影响，使分类器更依赖
        feature direction。默认关闭，因此不改变原始训练行为。
        """
