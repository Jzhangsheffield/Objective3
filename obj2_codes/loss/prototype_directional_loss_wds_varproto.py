...
"""
prototype_directional_loss_varproto.py
...
def _flatten_proto_bank(
    ...
    if prototype_bank.ndim != 3:
        raise ValueError(f"Expect [C, M, D], got {tuple(prototype_bank.shape)}")
...
# ============================================================
# 构造“可微分的 EMA 更新后 prototype bank 预览版”
# ============================================================
...
def build_differentiable_ema_bank(
    ...
    的样本才会参与 preview 更新。
    """
    if old_prototype_bank.ndim != 3:
        raise ValueError(f"old_prototype_bank must be [C, M, D], got {tuple(old_prototype_bank.shape)}")
    if q.ndim != 2:
    ...
    if not (q.shape[0] == labels.shape[0] == proto_ids.shape[0]):
        raise ValueError("q, labels, proto_ids must share the same batch size")
...
def prototype_directional_loss(
    ...
    """
    计算 prototype 关系的方向性约束损失。

    该损失只做两件事：
    --------------------
    1) 同类 prototype 若变远，则惩罚
    2) 异类 prototype 若变近，则惩罚

    定义：
    ------
    设：
        D_old(i,j) = old prototype 距离
        D_new(i,j) = preview prototype 距离
    ...
    _, M, _ = old_prototype_bank.shape
...
# ============================================================
# 真实 prototype bank 的 no_grad EMA 更新
# ============================================================
...
def ema_update_prototype_bank_(
    ...
    """
    if prototype_bank.ndim != 3:
        raise ValueError(f"prototype_bank must be [C, M, D], got {tuple(prototype_bank.shape)}")
    if q.ndim != 2:
        raise ValueError(f"q must be [B, D], got {tuple(q.shape)}")
    if labels.ndim != 1 or proto_ids.ndim != 1:
        raise ValueError(
