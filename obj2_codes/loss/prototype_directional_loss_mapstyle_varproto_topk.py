...
def prototype_directional_loss(
    ...
    若 topk_diff_classes > 0，则每个 updated prototype 只和 old 距离最近的
    K 个“不同类别”形成异类约束。这里的 nearest class 由该类中任意有效
    prototype 到当前 updated prototype 的最小 old cosine distance 决定。
    ...
    old_dist, old_valid = compute_prototype_distance_matrix(old_prototype_bank) # old_dist: [P, P]， old_valid: [P]
    new_dist, new_valid = compute_prototype_distance_matrix(new_prototype_bank) # new_dist: [P, P]， new_valid: [P]

    updated_mask = updated_mask_2d.view(-1) # [P]
    valid_mask = old_valid & new_valid # [P]

    updated_ids = torch.nonzero(updated_mask, as_tuple=False).squeeze(1) # torch.nonzero(updated_mask) 会返回所有 True 的位置, [U]
    valid_ids = torch.nonzero(valid_mask, as_tuple=False).squeeze(1) # [V]
    ...
    same_class_mask = updated_cls.unsqueeze(1).eq(valid_cls.unsqueeze(0)) # [U, 1] * [1, V] -> [U, V]
    diff_class_mask = ~same_class_mask # [U, V]

    self_mask = updated_ids.unsqueeze(1).eq(valid_ids.unsqueeze(0)) # [U, 1] * [1, V] -> [U, V]
    same_class_mask = same_class_mask & (~self_mask) # [U, V]

    # ------------------------------------------------------------
    # Optional hard negative class filtering for the different-class term.
    #
    # 原始版本会约束所有不同类 prototype。若 topk_diff_classes > 0，
    # 则对每一个 updated prototype，只选择 old distance 最近的 K 个不同类别。
    # 注意：这里是按“类别”选 top-k，而不是按 prototype 选 top-k。
    # 选中类别后，该类别下所有有效 prototype 都参与 diff loss。
    # ------------------------------------------------------------
    selected_diff_class_mask = diff_class_mask # [U, V]
    num_selected_diff_classes_total = torch.tensor(0, device=device)

    if topk_diff_classes is not None and int(topk_diff_classes) > 0:
        k_cls = int(topk_diff_classes) # k
        selected_diff_class_mask = torch.zeros_like(diff_class_mask) # [U, V]
        selected_class_counter = 0

        # 这里循环的规模是 num_updated_proto，通常很小；相比全矩阵计算开销可以忽略。
        for row_idx in range(updated_ids.numel()): 
            candidate_cols = torch.nonzero(diff_class_mask[row_idx], as_tuple=False).squeeze(1) # [1, V] -> [CAN]
            ...
            # 每个候选类别使用该类别中最近 prototype 的 old distance 代表类别距离。
            class_min_dists = []
        ...
        num_selected_diff_classes_total = torch.tensor(
            int(selected_class_counter), device=device
        )
