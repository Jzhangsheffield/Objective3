...
6) 支持 top-k 异类类别筛选：只对最近 K 个异类类别进行变近惩罚和变远奖励
...
而本版本在此基础上，额外加入：
    ...
    (D) top-k 异类类别筛选     -> 可选地只约束最近的 K 个异类类别
...
def prototype_directional_loss(
    ...
    topk_diff_classes: Optional[int] = None,
) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    """
    计算 far-reward 版本的 prototype directional loss，并可选支持 top-k 异类类别筛选。

    这个函数比较 old prototype bank 与 new preview bank 中 prototype 两两距离的变化方向，
    然后构造三类项：

    1) 同类 prototype 变远 -> 惩罚
       loss_same = mean( relu(D_new - D_old - same_margin) )

    2) 异类 prototype 变近 -> 惩罚
       loss_diff = mean( relu(D_old - D_new - diff_margin) )

    3) 异类 prototype 变远 -> 奖励
       reward_diff_far = mean( relu(D_new - D_old - diff_reward_margin) )

    总损失为：
       loss_dir = same_weight * loss_same
                + diff_weight * loss_diff
                - diff_reward_weight * reward_diff_far

    新增 top-k 逻辑
    ---------------
    若 topk_diff_classes is None 或 <= 0：
        异类项使用所有 different-class prototype pair。

    若 topk_diff_classes > 0：
        对每一个 updated prototype，只选择 old bank 中距离最近的 K 个“异类类别”。
        注意这里是按“类别”选 top-k，而不是按 prototype 选 top-k：

        - 对某个 updated prototype i，先找所有不同类别的有效 prototype；
        - 对每个候选类别 c，用该类别内与 i 最近的 prototype old distance
          作为类别距离；
        - 选择类别距离最小的 K 个类别；
        - 被选中的类别下所有有效 prototype 都参与：
              (a) 异类变近惩罚 loss_diff
              (b) 异类变远奖励 reward_diff_far

    这样做的原因
    -------------
    如果 reward 作用于所有异类类别，它会鼓励所有类别持续全局拉远；
    对模糊动作识别来说，更合理的是只关注当前最容易混淆的近邻类别。
    因此 top-k mask 必须同时作用于“变近惩罚”和“变远奖励”，
    避免出现“只惩罚最近 K 类，但奖励所有异类”的不一致目标。
    """
    device = old_prototype_bank.device
    dtype = old_prototype_bank.dtype

    # old_dist / new_dist: [P, P]，P = C * M_max。
    # 距离定义为 cosine distance = 1 - cosine similarity。
    ...
    # 只有 old/new 都是非零有效 prototype 的槽位才参与方向比较。
    # 这样可以避免 padded prototype 或尚未初始化 prototype 引入虚假距离。
    ...
    if updated_ids.numel() == 0 or valid_ids.numel() <= 1:
        ...
        return zero, {
            ...
            "topk_diff_classes": torch.tensor(
                0 if topk_diff_classes is None else int(topk_diff_classes), device=device
            ),
            "num_selected_diff_classes_total": torch.tensor(0, device=device),
    ...
    # 只取被当前 batch 更新过的 prototype 作为 anchor rows，
    # 与所有有效 prototype columns 比较。
    old_rows = old_dist[updated_ids][:, valid_ids]   # [U, V]
    new_rows = new_dist[updated_ids][:, valid_ids]   # [U, V]

    C, M, _ = old_prototype_bank.shape

    # 根据 flat_id 反推出 class_id：flat_id = class_id * M + proto_id。
    updated_cls = torch.div(updated_ids, M, rounding_mode="floor")  # [U]
    valid_cls = torch.div(valid_ids, M, rounding_mode="floor")      # [V]

    same_class_mask = updated_cls.unsqueeze(1).eq(valid_cls.unsqueeze(0))  # [U, V]
    diff_class_mask = ~same_class_mask                                    # [U, V]

    # self-pair 的距离恒为 0，且没有方向变化意义，因此从同类与异类候选中都去掉。
    ...
    # ------------------------------------------------------------
    # Optional hard negative class filtering for different-class terms.
    #
    # selected_diff_class_mask 是最终用于异类项的 mask：
    # - 不启用 top-k 时，它等于 diff_class_mask，即所有异类 pair；
    # - 启用 top-k 时，它只保留每个 updated prototype 最近 K 个异类类别下的 pair。
    #
    # 注意：这个 mask 同时用于 loss_diff 和 reward_diff_far，保证惩罚与奖励
    # 作用在同一组近邻异类类别上。
    # ------------------------------------------------------------
    selected_diff_class_mask = diff_class_mask
    num_selected_diff_classes_total = torch.tensor(0, device=device)

    if topk_diff_classes is not None and int(topk_diff_classes) > 0:
        k_cls = int(topk_diff_classes)
        selected_diff_class_mask = torch.zeros_like(diff_class_mask)
        selected_class_counter = 0

        # 循环维度是当前 batch 真正更新过的 prototype 数 U，通常远小于 P。
        for row_idx in range(updated_ids.numel()):
            # candidate_cols 是当前 anchor row 中所有异类 valid prototype 的列索引。
            candidate_cols = torch.nonzero(diff_class_mask[row_idx], as_tuple=False).squeeze(1)
            if candidate_cols.numel() == 0:
                continue

            candidate_classes = valid_cls[candidate_cols]
            unique_classes = torch.unique(candidate_classes)
            if unique_classes.numel() == 0:
                continue

            # 每个候选类别用“该类别中最近 prototype 的 old distance”代表该类别距离。
            # 使用 old distance 而不是 new distance，是为了让 top-k 选择基于更新前的稳定状态，
            # 避免当前 batch 的梯度变化反过来影响候选类别选择。
            class_min_dists = []
            for cls_id in unique_classes.tolist():
                cls_id = int(cls_id)
                cls_cols = candidate_cols[candidate_classes == cls_id]
                class_min_dists.append(old_rows[row_idx, cls_cols].detach().min())

            class_min_dists_t = torch.stack(class_min_dists, dim=0)
            k_eff = min(k_cls, int(unique_classes.numel()))
            nearest_pos = torch.topk(class_min_dists_t, k=k_eff, largest=False).indices
            nearest_classes = unique_classes[nearest_pos]
            selected_class_counter += int(k_eff)

            # 选中一个类别后，该类别下所有有效 prototype 都参与异类项。
            for cls_id in nearest_classes.tolist():
                cls_id = int(cls_id)
                selected_diff_class_mask[row_idx] |= diff_class_mask[row_idx] & (valid_cls == cls_id)

        num_selected_diff_classes_total = torch.tensor(int(selected_class_counter), device=device)
    ...
    # 2) 异类变近 -> 惩罚
    # --------------------------------------------------------
    # old - new > 0 表示异类距离减小；超过 diff_margin 的部分会产生惩罚。
    diff_bad = F.relu((old_rows - new_rows) - diff_margin)
    diff_vals = diff_bad[selected_diff_class_mask]

    if diff_vals.numel() > 0:
        loss_diff = diff_vals.mean()
        diff_shift_mean = (new_rows[selected_diff_class_mask] - old_rows[selected_diff_class_mask]).mean().detach()
    ...
    # 3) 异类变远 -> 奖励
    # --------------------------------------------------------
    # new - old > 0 表示异类距离增大；超过 diff_reward_margin 的部分被视为 reward。
    # 这里使用 selected_diff_class_mask，而不是 diff_class_mask，保证 reward 也只作用于近邻异类类别。
    diff_good = F.relu((new_rows - old_rows) - diff_reward_margin)
    diff_reward_vals = diff_good[selected_diff_class_mask]
    ...
    # 4) 总方向损失
    # --------------------------------------------------------
    # 当 diff_reward_weight > 0 时，loss_dir 可能为负数，这是 far-reward 设计允许的。
    loss_dir = (
        same_weight * loss_same
        + diff_weight * loss_diff
        - diff_reward_weight * reward_diff_far
    ...
    stats = {
        ...
        # num_same_pairs / num_diff_pairs 是参与均值计算的候选 pair 数量；
        # num_*_active_pairs 是真正产生非零惩罚或奖励的 pair 数量。
        ...
        # top-k 相关统计：用于确认 hard negative class filtering 是否真的生效。
        "topk_diff_classes": torch.tensor(
            0 if topk_diff_classes is None else int(topk_diff_classes), device=device
        ),
        "num_selected_diff_classes_total": num_selected_diff_classes_total.detach(),
...
def differentiable_ema_directional_loss(
    ...
    2) 基于 old/new bank 计算 far-reward directional loss。

    新增参数
    --------
    topk_diff_classes:
        - None 或 <= 0：异类惩罚/奖励使用所有不同类别；
        - > 0：每个 updated prototype 只对最近 K 个异类类别计算
          异类变近惩罚和异类变远奖励。

    diff_reward_margin / diff_reward_weight:
        控制 far-reward 项。若 diff_reward_weight=0，则 reward 项不会影响总损失，
        该函数就退化为普通 directional loss，但仍可使用 top-k 异类类别筛选。
    """
    ...
    loss_dir, stats = prototype_directional_loss(
        ...
        topk_diff_classes=topk_diff_classes,
    )
...
def directional_stats_to_str(stats: Dict[str, torch.Tensor]) -> str:
    ...
    return (
        ...
        f"topk_diff_classes={_v('topk_diff_classes'):.0f}, "
        f"selected_diff_classes={_v('num_selected_diff_classes_total'):.0f}, "
